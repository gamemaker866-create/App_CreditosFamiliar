...
CATALOGO_GASTAR = [
    {"id": 1, "emoji": "🛋️", "nombre": "Librarte de una tarea por 1 día", "coste": 60, "limite_semanal": 1},
    ...
    {"id": 7, "emoji": "🧊", "nombre": "Camarero por un momento", "coste": 20, "limite_diario": 2},
    {"id": 8, "emoji": "🍕", "nombre": "El ingrediente prohibido", "coste": 20, "limite_diario": 2}    
]

CATALOGO_GANAR = [
    {"id": 101, "emoji": "🧺", "nombre": "Recoger tendedero", "recompensa": 15, "limite_diario": 2},
    ...
    if not amigos:
        ...
        for a in amigos:
            ...
            with st.expander(f"👤 {a.capitalize()} — Saldo: {data_a.get('creditos', 0)} cr"):
                # Transferir créditos
                monto = st.number_input(f"Transferir créditos a {a.capitalize()}", min_value=1, max_value=usr_data["creditos"], key=f"tr_{a}")
